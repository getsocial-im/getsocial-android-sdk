...
public class PostActivityFragment extends BaseFragment implements Callback<ActivityPost> {
	...
	private void doPost() {
		...
		boolean hasVideo = _viewContainer._hasVideo.isChecked() && _videoDescriptor != null;
		...
		if (hasVideo) {
			builder.withMediaAttachment(MediaAttachment.video(_videoDescriptor._video));
		}
		showLoading("Posting activity", "Wait...");
	...
	public void onFailure(GetSocialException exception) {
		_viewContainer._image.setEnabled(true);
		if (exception.getErrorCode() == ErrorCode.ACTIVITY_REJECTED) {
			hideLoading();
			Toast.makeText(getContext(), "Your activity was rejected during the moderation: " + exception.getMessage(), Toast.LENGTH_SHORT).show();
		} else {
