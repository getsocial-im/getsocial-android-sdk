...
public class GroupMembersFragment extends BaseSearchFragment<MembersQuery, GroupMember> {
    ...
    protected MembersQuery createQuery(final SearchObject searchObject) {
        return MembersQuery.ofGroup(_groupId).withName(searchObject.searchTerm);
    }
    ...
    public class GroupMemberViewHolder extends ViewHolder {
        ...
        public void openActions() {
            ...
            if ((_role == Role.ADMIN || _role == Role.OWNER) && (!_item.getId().equals(GetSocial.getCurrentUser().getId()) && _item.getMembership().getRole() != Role.OWNER)) {
                ...
                if (_item.getMembership().getStatus() == MemberStatus.APPROVAL_PENDING || _item.getMembership().getStatus() == MemberStatus.MEMBER) {
                    dialog.addAction(new ActionDialog.Action("Reject") {
                        @Override
                        public void execute() {
                            rejectMember();
                        }
                ...
                if (_item.getMembership().getStatus() == MemberStatus.APPROVAL_PENDING || _item.getMembership().getStatus() == MemberStatus.REJECTED) {
        ...
        private void approveMember() {
            UpdateGroupMembersQuery query = UpdateGroupMembersQuery.create(_groupId, UserIdList.create(_item.getId()))
                    .withMemberStatus(MemberStatus.MEMBER).withRole(_item.getMembership().getRole());
        ...
        private void rejectMember() {
            UpdateGroupMembersQuery query = UpdateGroupMembersQuery.create(_groupId, UserIdList.create(_item.getId()))
                    .withMemberStatus(MemberStatus.REJECTED).withRole(_item.getMembership().getRole());
            Communities.updateGroupMembers(query, result -> {
                updateItem(_item, result.get(0));
            }, error -> Toast.makeText(getContext(), error.toString(), Toast.LENGTH_SHORT).show());
        }
