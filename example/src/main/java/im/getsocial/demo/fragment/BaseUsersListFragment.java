...
public abstract class BaseUsersListFragment<Query> extends BaseSearchFragment<Query, User> {
    ...
    private Map<String, Boolean> _blocked = new HashMap<>();
    ...
    public class UserViewHolder extends ViewHolder {
        ...
        public void openActions() {
            ...
            if (!GetSocial.getCurrentUser().getId().equals(userId)) {
                ...
                }).addAction(new ActionDialog.Action("Block") {
                    @Override
                    public void execute() {
                        Communities.blockUsers(UserIdList.create(userId),
                                () -> {
                                    Toast.makeText(getContext(), "Blocked " + _item.getDisplayName(), Toast.LENGTH_SHORT).show();
                                },
                                BaseUsersListFragment.this::onError);
                    }
                }).addAction(new ActionDialog.Action("Unblock") {
                    @Override
                    public void execute() {
                        Communities.unblockUsers(UserIdList.create(userId),
                                () -> {
                                    Toast.makeText(getContext(), "Unblocked " + _item.getDisplayName(), Toast.LENGTH_SHORT).show();
                                },
                                BaseUsersListFragment.this::onError);
                    }
                });
