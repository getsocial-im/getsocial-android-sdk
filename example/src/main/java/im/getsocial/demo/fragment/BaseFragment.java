...
public abstract class BaseFragment extends Fragment implements HasTitle, HasFragmentTag {
	...
	protected void loginWithFacebook(CompletionCallback callback) {
		final CompletionCallback wrapped = new CompletionCallback() {
			@Override
			public void onSuccess() {
				callback.onSuccess();
		...
		final AccessTokenTracker accessTokenTracker = new AccessTokenTracker() {
			...
			protected void onCurrentAccessTokenChanged(AccessToken oldAccessToken, @Nullable AccessToken newAccessToken) {
				...
				if (newAccessToken == null) {
					wrapped.onFailure(new GetSocialException(ErrorCode.UNKNOWN, "Facebook SDK did not provide an AccessToken"));
					return;
				}
				...
				GetSocial.init(AuthIdentity.createFacebookIdentity(tokenString), callback);
			}
		};
		accessTokenTracker.startTracking();
		LoginManager.getInstance().logInWithReadPermissions(getActivity(), FACEBOOK_PERMISSIONS);
	...
	protected void loginWithCustom(CompletionCallback callback) {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(getContext())
						...
						.setPositiveButton("Add", new DialogInterface.OnClickListener() {
									...
									public void onClick(DialogInterface dialog, int which) {
										...
										GetSocial.init(AuthIdentity.createCustomIdentity(providerId, userId, token), callback);
									}
								}
						)
						.setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
